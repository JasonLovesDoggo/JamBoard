...
def create_bounding_box_centers(start, end, steps):
    ...
    for i in range(steps):
        bounding_box_centers.append(
            [int(x1 + i * x_interval), int(y1 + i * y_interval)]
        )
...
def check_if_finger_tip_is_in_bounding_box(finger_tip, bounding_box, bounding_box_size):
    return (
        finger_tip[0] >= bounding_box[0] - bounding_box_size
        and finger_tip[0] <= bounding_box[0] + bounding_box_size
        and finger_tip[1] >= bounding_box[1] - bounding_box_size
        and finger_tip[1] <= bounding_box[1] + bounding_box_size
    )
